...
const createSchema = yup.object().shape({
    name: yup.string().min(2, 'Nom trop court*').max(100, 'Nom trop long*').required('Le nom est requis*'),
    description: yup.string().min(3, 'Description trop court*').max(300, 'Description trop long*').required('La description est requis*'),
    address: yup.string().min(3, 'L\'adresse trop court*').max(300, 'L\'adresse trop long*').required('L\'adresse est requis*'),
    date: yup.date().min(currentDate, 'La date ne peut pas être inférieure à la date actuelle').required('La date est requise*')
    ...
    time: yup.string().matches(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, 'L\'heure doit être au format HH:mm').required('L\'heure est requise'),
    image: yup.mixed()
        .optional()
...
export const actions: Actions = {
    createEvent: async ({ request, fetch }) => {
        ...
        const errors: { error?: string; name?: string; description?: string; date?: string; address?: string; image?: string; inviteList?: string; time?: string } = {};

        try {
            // Validation schema
            await createSchema.validate({ name, description, date, time, address, image, arrayInviteList }, { abortEarly: false });
        } catch (error) {
            // - Catch Errors
